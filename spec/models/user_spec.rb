  ...
  describe 'ユーザーモデルテスト' do
    ...
    context '名前バリデーション' do
      it 'nameが空ならバリデーションが通らない' do
        user = User.new(email: 'user@example.com', employee_number: 100_001, password: 'password')
        expect(user).not_to be_valid
      end
      it 'nameが26文字以上ならバリデーションが通らない' do
        user = User.new(name: 'a' * 26, email: 'user@example.com', employee_number: 100_001, password: 'password')
        expect(user).not_to be_valid
      end
    end
    context 'パスワードバリデーション' do
      it 'passwordが空ならバリデーションが通らない' do
        user = User.new(name: '失敗テスト', email: 'user@example.com', employee_number: 100_001)
        expect(user).not_to be_valid
      end
    end
    ...
    context 'バリデーションクリア' do
      it 'name,email,passwordをフォーマットに従って入力した場合、バリデーションが通る' do
        user = User.new(name: '成功テスト', email: 'user@example.com', employee_number: 100_001, password: 'password')
